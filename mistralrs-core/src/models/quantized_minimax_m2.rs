...
impl FusedMoe {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        // MiniMax applies routing bias BEFORE softmax (e_score_correction)
        let router_logits = if let Some(bias) = &self.routing_bias {
...
pub(crate) struct PropsGGUF {
    ...
    pub moe_cfg: MinimaxM2MoEConfig,
}

fn verify_minimax_m2_arch(
    ...
    if actual_arch != "minimax-m2" {
        candle_core::bail!("Expected `minimax-m2` architecture, got `{actual_arch}`.");
    }
    Ok(actual_arch)
}
...
impl TryFrom<ContentMetadata<'_>> for PropsGGUF {
    ...
    fn try_from(c: ContentMetadata) -> std::result::Result<Self, Self::Error> {
        let _ = verify_minimax_m2_arch(c.metadata)?;
        ...
        let moe_cfg = MinimaxM2MoEConfig {
            moe_intermediate_size: c.get_value::<u32>("feed_forward_length")? as usize,
        ...
        let props = Self {
            ...
            rope_freq_base: c.get_value("rope.freq_base").ok().unwrap_or(5_000_000_f32),
            key_length: c
...
/// Load the routing bias tensor if present (e_score_correction in HuggingFace)
fn load_routing_bias(
    ct: &mut Content<'_, impl std::io::Seek + std::io::Read>,
    prefix: &str,
    device: &Device,
) -> Result<Option<Tensor>> {
    let bias_name = format!("{prefix}.exp_probs_b.bias");
    if ct.has_tensor(&bias_name) {
        let qtensor: QTensor = ct.tensor(&bias_name, device)?;
        Ok(Some(qtensor.dequantize(device)?))
    } else {
        Ok(None)
    }
}
...
impl ModelConfig::FromGGUF for ModelWeights {
    fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        let actual_arch = verify_minimax_m2_arch(meta)?;
        ...
        for layer_idx in NiceProgressBar::<_, 'b'>(
            ...
            // MiniMax always uses MoE (no dense layers)
            ...
            let routing_bias = load_routing_bias(&mut ct, &prefix, device)?;
            ...
            // MiniMax uses shared QK norms (not per-head)
